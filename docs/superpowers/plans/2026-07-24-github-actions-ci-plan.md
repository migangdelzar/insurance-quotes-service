...
- The full-stack workflow intentionally smoke-tests the backend branch with the frontend repository's `main`; it starts from clean Compose volumes and verifies all three seeded password logins through the same-origin `/api` proxy. Feature-branch Playwright journeys were verified locally against both current worktrees.
